...
Public Class BBA_Exam_Panel
    ...
    Public Sub Next_question(ByVal i As Integer) 'prcedure to display questions from database
        ...
        command.CommandText = "Select * from tbl_bba_question where question_id = " & i & ""
        ...
        connection.Close()
    End Sub
    Private Sub bba_Exam_Panel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
Label:
    ...
    Sub Calculate_marks() ' procedure to calculate marks and insert into result table in databse
        ...
        command.CommandText = "select sum(marks) from tbl_answer
                               inner join tbl_bba_question on tbl_answer.answer_id = tbl_bba_question.question_id 
                               where tbl_answer.selected_answer = tbl_bba_question.answer "
        If connection.State = ConnectionState.Closed Then
            connection.Open()
        End If
        ...
        If marks > 5 Then
            command.CommandText = "insert into tbl_report values('" & login_id & "','Pass','BBA')"
            command.ExecuteNonQuery()
        ...
        Else
            command.CommandText = "insert into tbl_report values('" & login_id & "','Fail','BBA')"
            command.ExecuteNonQuery()
            MessageBox.Show("Calculating Result!!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Thread.Sleep(500)
            Fail_form.Show()
...
'command.CommandText = "Select max(question_id) from tbl_bba_question"
'connection.Open()
'If command.ExecuteScalar Is DBNull.Value Then
'    last_question = 1
'Else
