...
Public Class BCA_Exam_Panel
    ...
    Public Sub Next_question(ByVal i As Integer) 'prcedure to display questions from database
        ...
        command.CommandText = "Select * from tbl_bca_question where question_id = " & i & ""
        ...
        connection.Close()
    End Sub
    Private Sub BCA_Exam_Panel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
Label:
    ...
    Sub Calculate_marks() ' procedure to calculate marks and insert into result table in databse
        ...
        command.CommandText = "select sum(marks) from tbl_answer
                               inner join tbl_bca_question on tbl_answer.answer_id = tbl_bca_question.question_id 
                               where tbl_answer.selected_answer = tbl_bca_question.answer "
        If connection.State = ConnectionState.Closed Then
            connection.Open()
        End If
        ...
        If marks > 5 Then
            command.CommandText = "insert into tbl_report values('" & login_id & "','Pass','BCA')"
            command.ExecuteNonQuery()
        ...
        Else
            command.CommandText = "insert into tbl_report values('" & login_id & "','Fail','BCA')"
            command.ExecuteNonQuery()
            MessageBox.Show("Calculating Result!!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Thread.Sleep(500)
            Fail_form.Show()
